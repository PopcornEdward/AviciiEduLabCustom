 ...
 * @param {string} query 要显示的提示信息
 * @returns {Promise<string>} 用户的输入
 */
 ...
 * 格式化日期对象
 * @param {Date} date 
 * @returns {{dateString: string, pathString: string, timeStamp: number}}
 */
function formatDate(date) {
    ...
    // 用于目录结构: YYYY/MM/DD
    const pathString = `${year}/${month}/${day}`;
...
async function createPost() {
    ...
    try {
        ...
        while (selectedIndex < 0 || selectedIndex >= collections.length) {
            const input = await prompt(`请选择要创建文章的目录编号 (1-${collections.length}): `);
        ...
        let mode = 0;
        while (mode !== 1 && mode !== 2) {
            const input = await prompt('请选择模式编号 (1/2): ');
        ...
        // 3. 确定日期和文件信息
        let targetDate; // Date object
        let datePath;   // YYYY/MM/DD
        let titleInput;
        let fileSuffix = ''; // 用于模式1的时间戳后缀
        ...
        if (mode === 1) {
            ...
            while (!dateIsValid) {
                ...
                // 转换成 Date 对象
                // 注意：月份在 JS Date 中是 0-11，所以要减 1
                const inputDateObject = new Date(parseInt(parts[0]), parseInt(parts[1]) - 1, parseInt(parts[2]));
                
                // 规范化输入日期时间为午夜，用于精确比较
                const inputDateMidnight = new Date(inputDateObject.getFullYear(), inputDateObject.getMonth(), inputDateObject.getDate());

                // 检查是否为未来日期
                if (inputDateMidnight.getTime() > todayMidnight.getTime()) {
                    console.error('❌ 错误: 不能创建未来日期的文章。请重新输入今天的日期或过去的日期。');
                    dateInput = undefined; // 重置输入，继续循环
                    continue;
                }

                // 日期验证通过
                datePath = dateInput;
                targetDate = inputDateObject;
                dateIsValid = true;
            }
        }
        ...
        const fileSlug = toSlug(finalTitle);


        // 5. 构造路径和文件内容
        const BASE_COLLECTION_DIR = path.resolve('src', 'content', collectionName);
        const TARGET_DIR = path.join(BASE_COLLECTION_DIR, datePath);
        // 最终的文件名 = slug + 模式后缀 (.md)
        const FILENAME = `${fileSlug}${fileSuffix}.md`;
        const TARGET_FILE_PATH = path.join(TARGET_DIR, FILENAME);
...
请编辑此文件并开始写作。
        ...
        console.error('在创建文件时发生错误:', error.message);
    } finally {
